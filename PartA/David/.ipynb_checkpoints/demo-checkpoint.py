...
#for n in range(start_step + 1, 141):   
    ...
    if beacon_visible[n]:
        ...
        #plot_beconrange(axes, beacon_loc, beacon_pose, colour='green')
        ...
        if sum(weights) < 1e-50:
            ...
            Curr_pose = poses.mean(axis=0)
            Xmin = Curr_pose[0] - 0.5
            Xmax = Curr_pose[0] + 0.5
            Ymin = Curr_pose[1] - 0.5
            Ymax = Curr_pose[1] + 0.5
            Tmin = Curr_pose[2] - 0.1
            Tmax = Curr_pose[2] + 0.1

            weights = np.ones(Nparticles)
            #poses = np.zeros((Nparticles, 3))

            for m in range(Nparticles):
                poses[m] = (uniform(Xmin, Xmax), uniform(Ymin, Ymax), uniform(Tmin, Tmax))
    ...
    if n > display_step_prev + display_steps:
        ...
        #leave breadcrups SLAM
        #plot_path(axes, slam_poses[n], 'b.')
