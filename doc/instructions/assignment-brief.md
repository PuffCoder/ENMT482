...
Your task is to choose three of the sensors (not including sonar2) and use the measurements from these three sensors along with the commanded speed to estimate the robot's position.

You are provided with three datasets as CSV files, each containing true position, commanded velocity, and the sensor measurements:

* `calibration.csv` is for determining the sensor models
* `training1.csv` and `training2.csv` are for determining the motion models.

Additionally, you are provided with a test dataset (`test.csv`) which does not contain the true position, which you will use for your report.  Some Python example code is provided to read these files, but you may use another language or a spreadsheet if you prefer (although we may not be able to help).

To fuse the data you will need to determine a probabilistic sensor
model for each sensor and a probabilistic motion model for the
...
3. Postulate a function, $h(x)$, to fit the data.

4. Plot the residuals $z - h(x)$.   A good fit should have a zero-mean error.   Note, some sensors have outliers that you will need to exclude.

5. Determine the variance of $V$ from the residuals (use the `var()` function).
...
By the submission deadline you need to submit your code, along with one short report (six A4 sides max., 12pt, excluding title page) and a peer assessment each (half A4 side max.).
